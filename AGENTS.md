...
- 后续在本项目中写路径时，默认使用相对路径，优先相对项目根目录或 `TLM/` 根目录表达；只有在必须说明服务器挂载点时，才额外补充 Linux 绝对路径示例。
- 文档、命令示例、接口说明、结果说明里，不要把 `/root/data/...` 或 Windows 绝对路径作为主路径表达。
- 只要修改了接口、目录结构、运行命令、实验方案、结果读取方式或关键结论，就必须同步维护项目根目录下的 `CONTEXT.md`。
- 如果旧 README、阶段性说明文档和当前源码不一致，更新代码时要同时更新 `CONTEXT.md`，并以当前源码和当前主入口脚本为准。
